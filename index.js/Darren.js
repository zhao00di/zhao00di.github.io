...
window.onload=function(){
	...
	var aTimeP=getByClass(oMain,"time_name");//音乐名
	var aTimeAudio=document.getElementById("audio");//歌曲
	...
	function main_home(obj){
		...
		obj.onmouseout=function(ev){
			...
			var oMainBg=this.children[0];
			move(oMainBg,{left:0,top:oMainBg.offsetHeight});
		};
	};
	...
	function updata(){
		...
		for(var i=0;i<aImg2.length;i++){
			aImg2[i].src=str2.charAt(i)+'.png';
		}
		oWeek.src=week[w]+'.png';
	};
	...
	oPen.onclick=function(){
		if(!aMus){
			oPen.innerHTML="time关";
			aTimeAudio.play();
			aMus=true;
		}else{
			oPen.innerHTML="time开";
			aTimeAudio.pause();
			aMus=false;
		}		
	};*///time开关
	...
	function txt(){
		if(oMainA[3].className=="clear nav_active"){
			if(aAboutP.children.length!=0){
				...
				var str="欢迎访问Darren的个人网站，我是前端工程师，热衷于学习前端各种新技术，如果我没在找bug，那么就是在去找bug的路上...";
...
function circular(){
	...
	for(var i=0;i<N;i++){
		var oSpan=document.createElement("span");
		oSpan.innerHTML='<img src="自己的圆_files/'+(i+1)+'.jpg"/>';
